    ...
    for (int y = 256; y >= -255; --y) {
      ...
      for (int x = -255; x <= 256; ++x) {
        ...
        for (int sample = 0; sample < AA_samples; ++sample) {
          ...
          /// Depth-of-field
          Vector sensor_shift{random_offset()*0.1f,
                              random_offset()*0.1f,
                              0};
        ...
        outfile 
        << static_cast<unsigned char>(std::max(0.f, std::min(255.f, AA_color.x)))
        << static_cast<unsigned char>(std::max(0.f, std::min(255.f, AA_color.y)))
        << static_cast<unsigned char>(std::max(0.f, std::min(255.f, AA_color.z)));
      }
    }
